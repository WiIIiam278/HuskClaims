...
public class OldClaimWorld {
    ...
    private OldClaimWorld(@NotNull HuskClaims plugin) {
        this.id = 0;
    ...
    public static OldClaimWorld convert(int id, @NotNull Set<Claim> claims,
                                     @NotNull Map<UUID, String> userCache, @NotNull Set<OperationType> wildernessFlags) {
        final OldClaimWorld world = new OldClaimWorld();
        world.id = id;
        world.userCache = new ConcurrentHashMap<>(userCache);
        world.wildernessFlags = Sets.newConcurrentHashSet(wildernessFlags);
        world.claims = new CopyOnWriteArraySet<>(claims);
        return world;
    }

    @NotNull
    public static OldClaimWorld create(@NotNull HuskClaims plugin) {
        return new OldClaimWorld(plugin);
    }
...
//    }
    ...
    public boolean equals(Object obj) {
        return obj instanceof OldClaimWorld world && world.id == id;
    }
