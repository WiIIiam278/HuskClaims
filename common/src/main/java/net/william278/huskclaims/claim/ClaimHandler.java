...
public interface ClaimHandler extends Handler {
    ...
    default void showTaxInfoIfOwner(@NotNull OnlineUser user, @NotNull Claim claim, @NotNull ClaimWorld world) {
        ...
        // Get user's tax balance (database calls are synchronous and safe)
        try {
            ...
            if (totalOwed > 0.01) { // Use threshold to avoid floating point issues
            ...
            getPlugin().log(java.util.logging.Level.WARNING, "Error showing tax info on claim entry", e);
        }
    }
